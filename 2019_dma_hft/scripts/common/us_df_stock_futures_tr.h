...
typedef struct _DF_STOCK_FUT_EXEC {
	...
	char info_clsf[3];	// 정보구분, 01:개별주식선물
	char mrk_clsf;	// 시장구분, 5
	char isin_clsf[13];	// 종목코드
	int stk_snr;	// 종목일련번호
	char brd_id[3];	// 보드ID, 코드값모음참조
	char cur_prc_sign;	// 현재가격부호
	int cur_prc;	// 현재가격
	int exe_q;	// 체결수량
	char ss_id[3];	// 세션ID, 코드값모음참조
	char exe_t[9];	// 체결시각
	int rct_prd_prc;	// 최근월물의제약정가격
	int nxt_prd_prc;	// 원월물의제약정가격
	char init_prc_sign;	// 시가부호
	int init_prc;	// 시가
	char high_prc_sign;	// 고가부호
	int high_prc;	// 고가
	char low_prc_sign;	// 저가부호
	int low_prc;	// 저가
	char pri_prc_sign;	// 직전가격부호
	int pri_prc;	// 직전가격
	int accm_exe_q;	// 누적체결수량
	double accm_trd_amt;	// 누적거래대금, 단위:원
	char l_or_s;	// 최종매도매수구분코드, 1:매도,2:매수(최종으로 들어온 호가의 매도매수구분값)
	char real_t_high_prc_sign;	// 실시간상한가부호
	int real_t_high_prc;	// 실시간상한가, 개발가이드참조
	char real_t_low_prc_sign;	// 실시간하한가부호
	int real_t_low_prc;	// 실시간하한가, 개발가이드참조
} DF_STOCK_FUT_EXEC;
...
typedef struct _DF_STOCK_FUT_LIMIT_ORD {
	...
	char brd_id[3];	// 보드ID, 코드값모음참조
	char ss_id[3];	// 세션ID, 코드값모음참조
	int total_l_limit_ord_q;	// 매수총호가잔량, 자리수 변경(7->8)
	char l_lvl_1_sign;	// 매수1단계부호
	int l_lvl_1_prc;	// 매수1단계우선호가가격
	int l_lvl_1_q;	// 매수1단계우선호가잔량, 자리수 변경(6->7)
	char l_lvl_2_sign;	// 매수2단계부호
	int l_lvl_2_prc;	// 매수2단계우선호가가격
	int l_lvl_2_q;	// 매수2단계우선호가잔량, 자리수 변경(6->7)
	char l_lvl_3_sign;	// 매수3단계부호
	int l_lvl_3_prc;	// 매수3단계우선호가가격
	int l_lvl_3_q;	// 매수3단계우선호가잔량, 자리수 변경(6->7)
	char l_lvl_4_sign;	// 매수4단계부호
	int l_lvl_4_prc;	// 매수4단계우선호가가격
	int l_lvl_4_q;	// 매수4단계우선호가잔량, 자리수 변경(6->7)
	char l_lvl_5_sign;	// 매수5단계부호
	int l_lvl_5_prc;	// 매수5단계우선호가가격
	int l_lvl_5_q;	// 매수5단계우선호가잔량, 자리수 변경(6->7)
	char l_lvl_6_sign;	// 매수6단계부호
	int l_lvl_6_prc;	// 매수6단계우선호가가격
	int l_lvl_6_q;	// 매수6단계우선호가잔량, 자리수 변경(6->7)
	char l_lvl_7_sign;	// 매수7단계부호
	int l_lvl_7_prc;	// 매수7단계우선호가가격
	int l_lvl_7_q;	// 매수7단계우선호가잔량, 자리수 변경(6->7)
	char l_lvl_8_sign;	// 매수8단계부호
	int l_lvl_8_prc;	// 매수8단계우선호가가격
	int l_lvl_8_q;	// 매수8단계우선호가잔량, 자리수 변경(6->7)
	char l_lvl_9_sign;	// 매수9단계부호
	int l_lvl_9_prc;	// 매수9단계우선호가가격
	int l_lvl_9_q;	// 매수9단계우선호가잔량, 자리수 변경(6->7)
	char l_lvl_10_sign;	// 매수10단계부호
	int l_lvl_10_prc;	// 매수10단계우선호가가격
	int l_lvl_10_q;	// 매수10단계우선호가잔량, 자리수 변경(6->7)
	int total_s_limit_ord_q;	// 매도총호가잔량, 자리수 변경(7->8)
	char s_lvl_1_sign;	// 매도1단계부호
	int s_lvl_1_prc;	// 매도1단계우선호가가격
	int s_lvl_1_q;	// 매도1단계우선호가잔량, 자리수 변경(6->7)
	char s_lvl_2_sign;	// 매도2단계부호
	int s_lvl_2_prc;	// 매도2단계우선호가가격
	int s_lvl_2_q;	// 매도2단계우선호가잔량, 자리수 변경(6->7)
	char s_lvl_3_sign;	// 매도3단계부호
	int s_lvl_3_prc;	// 매도3단계우선호가가격
	int s_lvl_3_q;	// 매도3단계우선호가잔량, 자리수 변경(6->7)
	char s_lvl_4_sign;	// 매도4단계부호
	int s_lvl_4_prc;	// 매도4단계우선호가가격
	int s_lvl_4_q;	// 매도4단계우선호가잔량, 자리수 변경(6->7)
	char s_lvl_5_sign;	// 매도5단계부호
	int s_lvl_5_prc;	// 매도5단계우선호가가격
	int s_lvl_5_q;	// 매도5단계우선호가잔량, 자리수 변경(6->7)
	char s_lvl_6_sign;	// 매도6단계부호
	int s_lvl_6_prc;	// 매도6단계우선호가가격
	int s_lvl_6_q;	// 매도6단계우선호가잔량, 자리수 변경(6->7)
	char s_lvl_7_sign;	// 매도7단계부호
	int s_lvl_7_prc;	// 매도7단계우선호가가격
	int s_lvl_7_q;	// 매도7단계우선호가잔량, 자리수 변경(6->7)
	char s_lvl_8_sign;	// 매도8단계부호
	int s_lvl_8_prc;	// 매도8단계우선호가가격
	int s_lvl_8_q;	// 매도8단계우선호가잔량, 자리수 변경(6->7)
	char s_lvl_9_sign;	// 매도9단계부호
	int s_lvl_9_prc;	// 매도9단계우선호가가격
	int s_lvl_9_q;	// 매도9단계우선호가잔량, 자리수 변경(6->7)
	char s_lvl_10_sign;	// 매도10단계부호
	int s_lvl_10_prc;	// 매도10단계우선호가가격
	int s_lvl_10_q;	// 매도10단계우선호가잔량, 자리수 변경(6->7)
	...
	int l_lvl_6_trd_cnt;	// 매수6단계우선호가건수
	int l_lvl_7_trd_cnt;	// 매수7단계우선호가건수
	int l_lvl_8_trd_cnt;	// 매수8단계우선호가건수
	int l_lvl_9_trd_cnt;	// 매수9단계우선호가건수
	int l_lvl_10_trd_cnt;	// 매수10단계우선호가건수
	...
	int s_lvl_6_trd_cnt;	// 매도6단계우선호가건수
	int s_lvl_7_trd_cnt;	// 매도7단계우선호가건수
	int s_lvl_8_trd_cnt;	// 매도8단계우선호가건수
	int s_lvl_9_trd_cnt;	// 매도9단계우선호가건수
	int s_lvl_10_trd_cnt;	// 매도10단계우선호가건수
	char ord_rcv_t[9];	// 호가접수시각
	int exp_prc;	// 예상체결가격, 단일가매매시만 제공\n (장상태구분코드:10,20,30,80)
...
typedef struct _DF_STOCK_FUT_CLOSE_PRODUCT {
	...
	int stk_snr;	// 종목일련번호
	char brd_id[3];	// 보드ID, 코드값모음참조
	char close_prc_sign;	// 종가부호
	int close_prc;	// 종가, * 거래무 종목의 경우 기준가 set
...
typedef struct _DF_STOCK_FUT_MRK_OP_SCHD {
	...
	int stk_snr;	// 종목일련번호, *개별종목 장운영 조치시 등록
	char prd_id[12];	// 상품ID, 코드값모음참조 - 주식선물 전체장에 대해서는 space처리\n 기초자산별 이벤트 발생시 상품ID입력
	char mrk_prd_id[4];	// 장운영상품그룹ID, FST : 코스피 주식선물\nFKQ : 코스닥 주식선물
	char brd_id[3];	// 보드ID, 코드값모음참조
	char brd_event_id[4];	// 보드이벤트ID, 코드값모음참조
...
typedef struct _DF_STOCK_FUT_PRC_LIMIT_ON {
	...
	char high_prc_sign;	// 상한가부호
	int high_prc;	// 상한가, 적용된 상한가\n예)9999999
	char low_prc_sign;	// 하한가부호
	int low_prc;	// 하한가, 적용된 하한가\n예)9999999
...
typedef struct _DF_STOCK_FUT_RECOVERY_DATA {
	...
	char isin_clsf[13];	// 종목코드
	int stk_snr;	// 종목일련번호
	char cur_prc_sign;	// 현재가부호
	int cur_prc;	// 현재가
	int rct_prd_prc;	// 최근월물 의제약정가격
	int nxt_prd_prc;	// 원월물 의제약정가격
	char init_prc_sign;	// 시가부호
	int init_prc;	// 시가
	char high_prc_sign;	// 고가부호
	int high_prc;	// 고가
	...
	double accm_trd_amt;	// 누적거래대금, 단위:원
	char l_or_s;	// 최종매도매수구분코드, 1:매도,2:매수(최종으로 들어온 호가의 매도매수구분값)
	char s_lvl_1_sign;	// SIGN부호
	int s_lvl_1_prc;	// 매도1호가
	int s_lvl_1_q;	// 매도1호가수량, 자리수 변경(6->7)
	int s_lvl_1_trd_cnt;	// 매도1호가건수
	char l_lvl_1_sign;	// SIGN부호
	int l_lvl_1_prc;	// 매수1호가
	int l_lvl_1_q;	// 매수1호가수량, 자리수 변경(6->7)
	int l_lvl_1_trd_cnt;	// 매수1호가건수
	char s_lvl_2_sign;	// SIGN부호
	int s_lvl_2_prc;	// 매도2호가
	int s_lvl_2_q;	// 매도2호가수량, 자리수 변경(6->7)
	int s_lvl_2_trd_cnt;	// 매도2호가건수
	char l_lvl_2_sign;	// SIGN부호
	int l_lvl_2_prc;	// 매수2호가
	int l_lvl_2_q;	// 매수2호가수량, 자리수 변경(6->7)
	int l_lvl_2_trd_cnt;	// 매수2호가건수
	char s_lvl_3_sign;	// SIGN부호
	int s_lvl_3_prc;	// 매도3호가
	int s_lvl_3_q;	// 매도3호가수량, 자리수 변경(6->7)
	int s_lvl_3_trd_cnt;	// 매도3호가건수
	char l_lvl_3_sign;	// SIGN부호
	int l_lvl_3_prc;	// 매수3호가
	int l_lvl_3_q;	// 매수3호가수량, 자리수 변경(6->7)
	int l_lvl_3_trd_cnt;	// 매수3호가건수
	char s_lvl_4_sign;	// SIGN부호
	int s_lvl_4_prc;	// 매도4호가
	int s_lvl_4_q;	// 매도4호가수량, 자리수 변경(6->7)
	int s_lvl_4_trd_cnt;	// 매도4호가건수
	char l_lvl_4_sign;	// SIGN부호
	int l_lvl_4_prc;	// 매수4호가
	int l_lvl_4_q;	// 매수4호가수량, 자리수 변경(6->7)
	int l_lvl_4_trd_cnt;	// 매수4호가건수
	char s_lvl_5_sign;	// SIGN부호
	int s_lvl_5_prc;	// 매도5호가
	int s_lvl_5_q;	// 매도5호가수량, 자리수 변경(6->7)
	int s_lvl_5_trd_cnt;	// 매도5호가건수
	char l_lvl_5_sign;	// SIGN부호
	int l_lvl_5_prc;	// 매수5호가
	int l_lvl_5_q;	// 매수5호가수량, 자리수 변경(6->7)
	int l_lvl_5_trd_cnt;	// 매수5호가건수
	char s_lvl_6_sign;	// SIGN부호
	int s_lvl_6_prc;	// 매도6호가
	int s_lvl_6_q;	// 매도6호가수량, 자리수 변경(6->7)
	int s_lvl_6_trd_cnt;	// 매도6호가건수
	char l_lvl_6_sign;	// SIGN부호
	int l_lvl_6_prc;	// 매수6호가
	int l_lvl_6_q;	// 매수6호가수량, 자리수 변경(6->7)
	int l_lvl_6_trd_cnt;	// 매수6호가건수
	char s_lvl_7_sign;	// SIGN부호
	int s_lvl_7_prc;	// 매도7호가
	int s_lvl_7_q;	// 매도7호가수량, 자리수 변경(6->7)
	int s_lvl_7_trd_cnt;	// 매도7호가건수
	char l_lvl_7_sign;	// SIGN부호
	int l_lvl_7_prc;	// 매수7호가
	int l_lvl_7_q;	// 매수7호가수량, 자리수 변경(6->7)
	int l_lvl_7_trd_cnt;	// 매수7호가건수
	char s_lvl_8_sign;	// SIGN부호
	int s_lvl_8_prc;	// 매도8호가
	int s_lvl_8_q;	// 매도8호가수량, 자리수 변경(6->7)
	int s_lvl_8_trd_cnt;	// 매도8호가건수
	char l_lvl_8_sign;	// SIGN부호
	int l_lvl_8_prc;	// 매수8호가
	int l_lvl_8_q;	// 매수8호가수량, 자리수 변경(6->7)
	int l_lvl_8_trd_cnt;	// 매수8호가건수
	char s_lvl_9_sign;	// SIGN부호
	int s_lvl_9_prc;	// 매도9호가
	int s_lvl_9_q;	// 매도9호가수량, 자리수 변경(6->7)
	int s_lvl_9_trd_cnt;	// 매도9호가건수
	char l_lvl_9_sign;	// SIGN부호
	int l_lvl_9_prc;	// 매수9호가
	int l_lvl_9_q;	// 매수9호가수량, 자리수 변경(6->7)
	int l_lvl_9_trd_cnt;	// 매수9호가건수
	char s_lvl_10_sign;	// SIGN부호
	int s_lvl_10_prc;	// 매도10호가
	int s_lvl_10_q;	// 매도10호가수량, 자리수 변경(6->7)
	int s_lvl_10_trd_cnt;	// 매도10호가건수
	char l_lvl_10_sign;	// SIGN부호
	int l_lvl_10_prc;	// 매수10호가
	int l_lvl_10_q;	// 매수10호가수량, 자리수 변경(6->7)
	int l_lvl_10_trd_cnt;	// 매수10호가건수
	int total_s_limit_ord_q;	// 매도 총호가수량, 자리수 변경(7->8)
	int total_s_trd_cnt;	// 매도유효호가건수
	int total_l_limit_ord_q;	// 매수 총호가수량, 자리수 변경(7->8)
	int total_l_trd_cnt;	// 매수유효호가건수
	char ss_id[3];	// 세션ID, 코드값모음참조
	char brd_id[3];	// 보드ID, 코드값모음참조
...
typedef struct _DF_STOCK_FUT_INVESTOR_DATA {
	...
	char prd_id[12];	// 상품ID, 코드값모음참조 - 시장전체(KRDRVFU999 )
...
typedef struct _DF_STOCK_FUT_PRD_INFO_DATA {
	...
	char prc_limit_exp_dir;	// 가격제한확대방향코드, 가격제한이 다단계로 정의되는 경우 다음 단계로 확대되는 요건을 충족할 때 확대를 수행하는 방향\n## 코드값##\nX:미적용\nF:정방향\nB:역방향\nT:양방향
	int prc_limit_final_stg;	// 가격제한확대적용방향코드, 가격제한이 다단계로 정의되는 경우 제한의 최종단계번호
	char prc_limit_stg_1_hgh_sign;	// 가격제한1단계SIGN부호
	double prc_limit_stg_1_high_prc;	// 가격제한1단계상한가, 예) 9999999999.99
	char prc_limit_stg_1_low_sign;	// 가격제한1단계SIGN부호
	double prc_limit_stg_1_low_prc;	// 가격제한1단계하한가, 예) 9999999999.99
	char prc_limit_stg_2_high_sign;	// 가격제한2단계SIGN부호
	double prc_limit_stg_2_high_prc;	// 가격제한2단계상한가, 예) 9999999999.99
	char prc_limit_stg_2_low_sign;	// 가격제한2단계SIGN부호
	double prc_limit_stg_2_low_prc;	// 가격제한2단계하한가, 예) 9999999999.99
	char prc_limit_stg_3_high_sign;	// 가격제한3단계SIGN부호
	double prc_limit_stg_3_high_prc;	// 가격제한3단계상한가, 예) 9999999999.99
	char prc_limit_stg_3_low_sign;	// 가격제한3단계SIGN부호
	...
	char yest_adj_close_prc_sign;	// SIGN부호
	...
	double limit_ord_upper_limit_q;	// 상한수량, 호가상한수량 (20181210 항목추가)
	double limit_ord_lower_limit_q;	// 하한수량, 호가하한수량 (20181210 항목추가)
	double block_trd_ord_upper_limit_q;	// 협의대량매매상한수량, 협의대량매매호가상한수량 (20181210 항목추가)
	double block_trd_ord_lower_limit_q;	// 협의대량매매하한수량, 협의대량매매호가하한수량 (20181210 항목추가)
} DF_STOCK_FUT_PRD_INFO_DATA;


// 주식선물_조정내역 데이터
typedef struct _DF_STOCK_FUT_ADJ_INFO_DATA {
	char data_clsf[3];	// DATA구분, H4
	char info_clsf[3];	// 정보구분, 01:개별주식선물
	char mrk_clsf;	// 시장구분, 5
	char trd_date[9];	// 영업일자
	char isin_clsf[13];	// 종목코드, 최종 record '999999999999' set
	double adj_coef_denominator;	// 조정계수분모, 예) 999999999999.99999999
	double adj_coef_numerator;	// 조정계수분자, 예) 999999999999.99999999
	double bef_adj_unit_size;	// 조정전 거래단위(계약크기), 예) 999999999999.99999999
	double aft_adj_unit_size;	// 조정후 거래단위(계약크기), 예) 999999999999.99999999
	double bef_adj_exercise_prc;	// 조정전 행사가격, 예) 999999999999.99999999
	double aft_adj_exercise_prc;	// 조정후 행사가격, 예) 999999999999.99999999
	int aft_adj_open_interest_adj_coef;	// 조정후 미결제수량 조정계수
