...
class BarSeries(object):
    ...
    def process_ticks(self, price_column='cur_price', volume_column='exe_q', time_column='time_diff', frequency='15Min'):

        ## 데이터 분할 간격 Default value == 15Min
        ## Frequency examples
        # 1M  == 1달
        # 1H == 1시간
        # 1Min == 1분
        # 1S = 1초
        # 1ms = 1밀리세컨드
        # 1U = 1마이크로세컨드 (1ms 이하부터는 memory error가 남 / 더 짧은 단위 시간을 사용하려면, resampling 대신 배열형태로 작성되는 구조적 변화가 필요함)
        # 1N = 1나노세컨드

        ## 시간 단위가 작아 질수록, 그 시간대에 데이터가 없을 수 있음. 그런 경우네느 df.dropna()해서 없는 시간대는 삭제해야함.
        ## main dataframe은 ohlc_df임. ohlc frame에 volume, time을 붙이는 형태임
...
class TickBarSeries(BarSeries):
    ...
    def process_ticks(self, price_column='cur_price', frequency='15Min'):
        ohlc_df = self.process_ohlc(price_column, frequency)
        return ohlc_df
...
class VolumeBarSeries(BarSeries):
    ...
    def process_ticks(self, price_column = 'cur_price', volume_column='exe_q', frequency='15Min'):
        ohlc_df = self.process_ohlc(price_column, frequency)
        return ohlc_df
...
class DollarBarSeries(BarSeries):
    ...
    def process_ticks(self, price_column='cur_price', volume_column='exe_q', frequency=10000):
        ohlc_df = self.process_ohlc(price_column, frequency)
        return ohlc_df
...
class ImbalanceTickBarSeries(BarSeries):
    ...
    def get_bt(self, data):
        ## np.sign 처음 사용해봄 => 유용해보임. Why? 값의 부호만 추출 가능하기 때문.
    ...
    def process_ticks(self, price_column = 'cur_price', volume_column='exe_q', init=100, min_bar = 10, max_bar = 1000):
