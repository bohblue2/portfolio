...
int ParseC4016(const double& timestamp, const char* in_buff, DF_COM_FUT_COMP_BLOCK* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mkt_clsf = data->mkt_clsf[0];	//시장구분,6
	memcpy(out->isin_clsf, data->isin_clsf, sizeof(data->isin_clsf));	// 종목코드,*개발가이드 참조 : 신국채선물 추가\n 2010.10.25
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목일련번호
	out->cpt_block_trd_vol = Str2Dbl(data->cpt_block_trd_vol, sizeof(data->cpt_block_trd_vol), 0);	// 경쟁+협의합산 거래량,경쟁매매, 협의대량거래, EFP, FLEX 거래를 합산한 거래량
	out->cpt_block_trd_amt = Str2Dbl(data->cpt_block_trd_amt, sizeof(data->cpt_block_trd_amt), 0);	// 경쟁+협의합산 거래대금,경쟁매매, 협의대량거래, EFP, FLEX 거래를 합산한 거래대금\n(단위 : 원)
...
int ParseA3016(const double& timestamp, const char* in_buff, DF_COM_FUT_EXEC* out) {
	...
	out->cur_prc = Str2SignDouble(data->cur_prc_sign, sizeof(data->cur_prc_sign) + sizeof(data->cur_prc), 2);	// 현재가격,예) 999999.99
	...
	out->rct_prd_prc = Str2Dbl(data->rct_prd_prc, sizeof(data->rct_prd_prc), 2);	// 최근월물의제약정가격,예) 999999.99
	out->nxt_prd_prc = Str2Dbl(data->nxt_prd_prc, sizeof(data->nxt_prd_prc), 2);	// 원월물의제약정가격,예) 999999.99
	out->init_prc = Str2SignDouble(data->init_prc_sign, sizeof(data->init_prc_sign) + sizeof(data->init_prc), 2);	// 시가,예) 999999.99
	out->high_prc = Str2SignDouble(data->high_prc_sign, sizeof(data->high_prc_sign) + sizeof(data->high_prc), 2);	// 고가,예) 999999.99
	out->low_prc = Str2SignDouble(data->low_prc_sign, sizeof(data->low_prc_sign) + sizeof(data->low_prc), 2);	// 저가,예) 999999.99
	out->pri_prc = Str2SignDouble(data->pri_prc_sign, sizeof(data->pri_prc_sign) + sizeof(data->pri_prc), 2);	// 직전가격,예) 999999.99
	...
	out->real_t_high_prc = Str2SignDouble(data->real_t_high_prc_sign, sizeof(data->real_t_high_prc_sign) + sizeof(data->real_t_high_prc), 2);	// 실시간상한가,예) 999999.99 개발가이드참조
	out->real_t_low_prc = Str2SignDouble(data->real_t_low_prc_sign, sizeof(data->real_t_low_prc_sign) + sizeof(data->real_t_low_prc), 2);	// 실시간하한가,예) 999999.99 개발가이드참조
...
int ParseB6016(const double& timestamp, const char* in_buff, DF_COM_FUT_LIMIT_ORD* out) {
	...
	out->l_lvl_1_prc = Str2SignDouble(data->l_lvl_1_sign, sizeof(data->l_lvl_1_sign) + sizeof(data->l_lvl_1_prc), 2);	// 매수1단계우선호가가격,예) 999999.99
	out->l_lvl_1_q = Str2Int(data->l_lvl_1_q, sizeof(data->l_lvl_1_q));	// 매수1단계우선호가잔량
	out->l_lvl_2_prc = Str2SignDouble(data->l_lvl_2_sign, sizeof(data->l_lvl_2_sign) + sizeof(data->l_lvl_2_prc), 2);	// 매수2단계우선호가가격,예) 999999.99
	out->l_lvl_2_q = Str2Int(data->l_lvl_2_q, sizeof(data->l_lvl_2_q));	// 매수2단계우선호가잔량
	out->l_lvl_3_prc = Str2SignDouble(data->l_lvl_3_sign, sizeof(data->l_lvl_3_sign) + sizeof(data->l_lvl_3_prc), 2);	// 매수3단계우선호가가격,예) 999999.99
	out->l_lvl_3_q = Str2Int(data->l_lvl_3_q, sizeof(data->l_lvl_3_q));	// 매수3단계우선호가잔량
	out->l_lvl_4_prc = Str2SignDouble(data->l_lvl_4_sign, sizeof(data->l_lvl_4_sign) + sizeof(data->l_lvl_4_prc), 2);	// 매수4단계우선호가가격,예) 999999.99
	out->l_lvl_4_q = Str2Int(data->l_lvl_4_q, sizeof(data->l_lvl_4_q));	// 매수4단계우선호가잔량
	out->l_lvl_5_prc = Str2SignDouble(data->l_lvl_5_sign, sizeof(data->l_lvl_5_sign) + sizeof(data->l_lvl_5_prc), 2);	// 매수5단계우선호가가격,예) 999999.99
	...
	out->s_lvl_1_prc = Str2SignDouble(data->s_lvl_1_sign, sizeof(data->s_lvl_1_sign) + sizeof(data->s_lvl_1_prc), 2);	// 매도1단계우선호가가격,예) 999999.99
	out->s_lvl_1_q = Str2Int(data->s_lvl_1_q, sizeof(data->s_lvl_1_q));	// 매도1단계우선호가잔량
	out->s_lvl_2_prc = Str2SignDouble(data->s_lvl_2_sign, sizeof(data->s_lvl_2_sign) + sizeof(data->s_lvl_2_prc), 2);	// 매도2단계우선호가가격,예) 999999.99
	out->s_lvl_2_q = Str2Int(data->s_lvl_2_q, sizeof(data->s_lvl_2_q));	// 매도2단계우선호가잔량
	out->s_lvl_3_prc = Str2SignDouble(data->s_lvl_3_sign, sizeof(data->s_lvl_3_sign) + sizeof(data->s_lvl_3_prc), 2);	// 매도3단계우선호가가격,예) 999999.99
	out->s_lvl_3_q = Str2Int(data->s_lvl_3_q, sizeof(data->s_lvl_3_q));	// 매도3단계우선호가잔량
	out->s_lvl_4_prc = Str2SignDouble(data->s_lvl_4_sign, sizeof(data->s_lvl_4_sign) + sizeof(data->s_lvl_4_prc), 2);	// 매도4단계우선호가가격,예) 999999.99
	out->s_lvl_4_q = Str2Int(data->s_lvl_4_q, sizeof(data->s_lvl_4_q));	// 매도4단계우선호가잔량
	out->s_lvl_5_prc = Str2SignDouble(data->s_lvl_5_sign, sizeof(data->s_lvl_5_sign) + sizeof(data->s_lvl_5_prc), 2);	// 매도5단계우선호가가격,예) 999999.99
	...
	out->exp_prc = Str2SignDouble(data->exp_prc_sign, sizeof(data->exp_prc_sign) + sizeof(data->exp_prc), 2);	// 예상체결가격,예) 999999.99 단일가매매시만 제공\n (장상태구분코드:10,20,30,80)
...
int ParseG7016(const double& timestamp, const char* in_buff, DF_COM_FUT_EXEC_LIMIT_ORD* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->isin_clsf, data->isin_clsf, sizeof(data->isin_clsf));	// 종목코드,*개발가이드 참조 : 신국채선물 추가\n 2010.10.25
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목일련번호
	memcpy(out->brd_id, data->brd_id, sizeof(data->brd_id));	// 보드ID,코드값모음참조
	out->cur_price = Str2SignDouble(data->cur_prc_sign, sizeof(data->cur_prc_sign) + sizeof(data->cur_price), 2);	// 현재가격,예) 999999.99
	out->exe_q = Str2Int(data->exe_q, sizeof(data->exe_q));	// 체결수량
	memcpy(out->ss_id, data->ss_id, sizeof(data->ss_id));	// 세션ID,코드값모음참조
	memcpy(out->exe_t, data->exe_t, sizeof(data->exe_t));	// 체결시각
	out->rct_prd_prc = Str2Dbl(data->rct_prd_prc, sizeof(data->rct_prd_prc), 2);	// 최근월물의제약정가격,예) 999999.99
...
int ParseA6016(const double& timestamp, const char* in_buff, DF_COM_FUT_CLOSE_PRODUCT* out) {
	...
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목일련번호
	memcpy(out->brd_id, data->brd_id, sizeof(data->brd_id));	// 보드ID,코드값모음참조
	out->close_prc = Str2SignDouble(data->close_prc_sign, sizeof(data->close_prc_sign) + sizeof(data->close_prc), 2);	// 종가,* 거래무 종목의 경우 기준가 set, 예) 999999.99
...
int ParseM4016(const double& timestamp, const char* in_buff, DF_COM_FUT_MRK_OP_SCHD* out) {
	...
	memcpy(out->stk_snr, data->stk_snr, sizeof(data->stk_snr));	// 종목일련번호,*개별종목 장운영 조치시 등록
	memcpy(out->prd_id, data->prd_id, sizeof(data->prd_id));	// 상품ID,코드값모음참조 : space처리
	memcpy(out->mrk_prd_id, data->mrk_prd_id, sizeof(data->mrk_prd_id));	// 장운영상품그룹ID,*코드값모음참조
	memcpy(out->brd_id, data->brd_id, sizeof(data->brd_id));	// 보드ID,코드값모음참조
	memcpy(out->brd_event_id, data->brd_event_id, sizeof(data->brd_event_id));	// 보드이벤트ID,코드값모음참조
	...
	memcpy(out->ss_id, data->ss_id, sizeof(data->ss_id));	// 세션ID,코드값모음참조
	out->adj_stage = Str2Int(data->adj_stage, sizeof(data->adj_stage));	// 적용단계,해당사항없음
	out->under_asset_prc_chg = data->under_asset_prc_chg[0];	//기준종목가격확대발생코드,해당사항없음
	memcpy(out->exp_prc_chg_t, data->exp_prc_chg_t, sizeof(data->exp_prc_chg_t));	// 가격확대예정시각,해당사항없음
...
int ParseH3016(const double& timestamp, const char* in_buff, DF_COM_FUT_SETT_PRC_DATA* out) {
	...
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->isin_clsf, data->isin_clsf, sizeof(data->isin_clsf));	// 종목코드,최종 record '999999999999' set\n*개발가이드 참조 : 신국채선물 추가
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목SEQ,돈육선물 - 최종거래일이 지난 종목은 0으로 전송
	out->calc_prc = Str2Dbl(data->calc_prc, sizeof(data->calc_prc), 8);	// 정산가격,돈육선물 - 최종거래일이 지난 종목은 0으로 전송\n예) 99999999.99999999
	out->calc_prc_clsf = data->calc_prc_clsf[0];	//정산가격구분,돈육선물 - 최종거래일이 지난 종목은 공백으로 전송\n0 : 정산가 없슴(최초 거래성립전 기세 포함)\n1 : 당일 종가(실세)\n2 : 당일 기세(최초 거래성립후 기세) - 2010.12.20 삭제\n3 : 전일 정산가(최초 거래성립후 종가 미형성) - 2010.12.20 삭제\n4 : 당일 이론가(최초 거래성립후 종가 미형성)\n5 : 스프레드거래에 의해 생성된 월물의 종가 - 2010.12.20 삭제\n6 : 조정된 전일 정산가 - 2010.12.20 삭제\n7 : 대상자산 종가(이론가없는상품)\n8 : 정산기준가격 - 2010.12.20 추가
...
int ParseH1016(const double& timestamp, const char* in_buff, DF_COM_FUT_INVESTOR_DATA* out) {
	...
	memcpy(out->prd_id, data->prd_id, sizeof(data->prd_id));	// 상품ID,코드값모음참조
...
int ParseB2016(const double& timestamp, const char* in_buff, DF_COM_FUT_RECOVERY_DATA* out) {
	...
	memcpy(out->isin_clsf, data->isin_clsf, sizeof(data->isin_clsf));	// 종목코드,*개발가이드 참조 : 신국채선물 추가
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목일련번호
	out->cur_prc = Str2SignDouble(data->cur_prc_sign, sizeof(data->cur_prc_sign) + sizeof(data->cur_prc), 2);	// 현재가,예) 999999.99
	out->rct_prd_prc = Str2Dbl(data->rct_prd_prc, sizeof(data->rct_prd_prc), 2);	// 최근월물의제약정가격,예) 999999.99
	out->nxt_prd_prc = Str2Dbl(data->nxt_prd_prc, sizeof(data->nxt_prd_prc), 2);	// 원월물의제약정가격,예) 999999.99
	out->init_prc = Str2SignDouble(data->init_prc_sign, sizeof(data->init_prc_sign) + sizeof(data->init_prc), 2);	// 시가,예) 999999.99
	out->high_prc = Str2SignDouble(data->high_prc_sign, sizeof(data->high_prc_sign) + sizeof(data->high_prc), 2);	// 고가,예) 999999.99
	...
	out->accm_trd_amt = Str2Dbl(data->accm_trd_amt, sizeof(data->accm_trd_amt), 0);	// 누적거래대금,단위:원
	out->l_or_s = data->l_or_s[0];	//최종매도매수구분코드,1:매도,2:매수(최종으로 들어온 호가의 매도매수구분값)
	out->s_lvl_1_prc = Str2SignDouble(data->s_lvl_1_sign, sizeof(data->s_lvl_1_sign) + sizeof(data->s_lvl_1_prc), 2);	// 매도1호가,예) 999999.99
	...
	out->l_lvl_1_prc = Str2SignDouble(data->l_lvl_1_sign, sizeof(data->l_lvl_1_sign) + sizeof(data->l_lvl_1_prc), 2);	// 매수1호가,예) 999999.99
	...
	out->s_lvl_2_prc = Str2SignDouble(data->s_lvl_2_sign, sizeof(data->s_lvl_2_sign) + sizeof(data->s_lvl_2_prc), 2);	// 매도2호가,예) 999999.99
	...
	out->l_lvl_2_prc = Str2SignDouble(data->l_lvl_2_sign, sizeof(data->l_lvl_2_sign) + sizeof(data->l_lvl_2_prc), 2);	// 매수2호가,예) 999999.99
	...
	out->s_lvl_3_prc = Str2SignDouble(data->s_lvl_3_sign, sizeof(data->s_lvl_3_sign) + sizeof(data->s_lvl_3_prc), 2);	// 매도3호가,예) 999999.99
	...
	out->l_lvl_3_prc = Str2SignDouble(data->l_lvl_3_sign, sizeof(data->l_lvl_3_sign) + sizeof(data->l_lvl_3_prc), 2);	// 매수3호가,예) 999999.99
	...
	out->s_lvl_4_prc = Str2SignDouble(data->s_lvl_4_sign, sizeof(data->s_lvl_4_sign) + sizeof(data->s_lvl_4_prc), 2);	// 매도4호가,예) 999999.99
	...
	out->l_lvl_4_prc = Str2SignDouble(data->l_lvl_4_sign, sizeof(data->l_lvl_4_sign) + sizeof(data->l_lvl_4_prc), 2);	// 매수4호가,예) 999999.99
	...
	out->s_lvl_5_prc = Str2SignDouble(data->s_lvl_5_sign, sizeof(data->s_lvl_5_sign) + sizeof(data->s_lvl_5_prc), 2);	// 매도5호가,예) 999999.99
	...
	out->l_lvl_5_prc = Str2SignDouble(data->l_lvl_5_sign, sizeof(data->l_lvl_5_sign) + sizeof(data->l_lvl_5_prc), 2);	// 매수5호가,예) 999999.99
...
int ParseA0016(const double& timestamp, const char* in_buff, DF_COM_FUTRE_PRD_INFO_DATA* out) {
	...
	memcpy(out->listing_date, data->listing_date, sizeof(data->listing_date));	// 상장일자,*개발가이드 내용추가	2009.08.31
	...
	out->prc_limit_exp_dir = data->prc_limit_exp_dir[0];	//가격제한확대적용방향코드,가격제한이 다단계로 정의되는 경우 다음 단계로 확대되는 요건을 충족할 때 확대를 수행하는 방향\n## 코드값##\nX:미적용\nF:정방향\nB:역방향\nT:양방향
	...
	out->efp_clsf = data->efp_clsf[0];	//EFP거래대상여부,Y, N, 상품선물만 해당  2009.08.31
	out->flex_clsf = data->flex_clsf[0];	//FLEX거래대상여부,Y, N, 상품선물만 해당  2009.08.31
	out->yest_efp_exe_q = Str2Dbl(data->yest_efp_exe_q, sizeof(data->yest_efp_exe_q), 0);	// 전일EFP체결수량,상품선물만 해당	   2009.08.31
	out->yest_efp_trd_amount = Str2Dbl(data->yest_efp_trd_amount, sizeof(data->yest_efp_trd_amount), 0);	// 전일EFP거래대금,상품선물만 해당	   2009.08.31
...
int ParseH5016(const double& timestamp, const char* in_buff, DF_GOV_BOND_CD_DATA* out) {
	...
	out->timestamp = timestamp;
	memcpy(out->data_clsf, data->data_clsf, sizeof(data->data_clsf));	// DATA구분,H5
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->send_date, data->send_date, sizeof(data->send_date));	// 전송일자,YYYYMMDD
	memcpy(out->send_time, data->send_time, sizeof(data->send_time));	// 전송시간,HHMMSS
	memcpy(out->uder_asset_clsf, data->uder_asset_clsf, sizeof(data->uder_asset_clsf));	// 기초자산구분,BM3,BM5,BMA,CD,MSB   2010.10.25\n*항목명 변경\n 기초자산ID->기초자산구분  2009.08.31\n*MSB제공중단		  2011.05.02
	out->final_ord_return_pctg = Str2Dbl(data->final_ord_return_pctg, sizeof(data->final_ord_return_pctg), 5);	// 최종호가수익률,999.99999\nCD(91일),국채(3년,5년,10년)\n*MSB제공중단		  2011.05.02
	memcpy(out->gov_bond_code, data->gov_bond_code, sizeof(data->gov_bond_code));	// 국채코드,국채선물의 결재기준채권코드
	memcpy(out->gov_bond_prd_code, data->gov_bond_prd_code, sizeof(data->gov_bond_prd_code));	// 국채선물종목코드
	memcpy(out->data_created_t, data->data_created_t, sizeof(data->data_created_t));	// 데이터 발생시간,HHMMSS
...
int ParseH6016(const double& timestamp, const char* in_buff, DF_STD_GOV_BOND_DATA* out) {
	...
	out->timestamp = timestamp;
	memcpy(out->data_clsf, data->data_clsf, sizeof(data->data_clsf));	// DATA구분,H6
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->adj_date, data->adj_date, sizeof(data->adj_date));	// 적용일,익일자로 전송됨
	memcpy(out->prd, data->prd, sizeof(data->prd));	// 상품,BM3,BM5,BMA   2010.10.25
	memcpy(out->fut_prd_code, data->fut_prd_code, sizeof(data->fut_prd_code));	// 선물종목코드,익일 거래 대상인 종목
	memcpy(out->under_asset_bond_code, data->under_asset_bond_code, sizeof(data->under_asset_bond_code));	// 채권코드,국채선물의 결재기준채권코드
	out->conv_coef = Str2Dbl(data->conv_coef, sizeof(data->conv_coef), 6);	// 전환계수,인수도결제 국채선물인 경우만 해당됨\n예) 99.999999\n'00000000' 처리   2010.10.25
...
int ParseH9016(const double& timestamp, const char* in_buff, DF_USD_DATA* out) {
	...
	out->timestamp = timestamp;
	memcpy(out->data_clsf, data->data_clsf, sizeof(data->data_clsf));	// DATA구분,H9
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->send_date, data->send_date, sizeof(data->send_date));	// 전송일자,YYYYMMDD
	memcpy(out->send_time, data->send_time, sizeof(data->send_time));	// 전송시간,HHMMSS
	memcpy(out->under_asset_id, data->under_asset_id, sizeof(data->under_asset_id));	// 기초자산ID,USD
	out->s_limit_ord_prc = Str2Dbl(data->s_limit_ord_prc, sizeof(data->s_limit_ord_prc), 2);	// 매도호가,예) 9999.99
	out->l_limit_ord_prc = Str2Int(data->l_limit_ord_prc, sizeof(data->l_limit_ord_prc));	// 매수호가
	out->yest_close_prc = Str2Int(data->yest_close_prc, sizeof(data->yest_close_prc));	// 전일종가
	...
	out->exe_prc = Str2Int(data->exe_prc, sizeof(data->exe_prc));	// 체결가
	memcpy(out->data_created_t, data->data_created_t, sizeof(data->data_created_t));	// 데이터 발생시간,HHMMSS
...
int ParseI0016_FBB1(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_1* out) {
	...
	out->timestamp = timestamp;
	memcpy(out->data_clsf, data->data_clsf, sizeof(data->data_clsf));	// DATA구분,I0
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FBB1(원/달러 전일시간대별 거래현황)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일
	memcpy(out->trd_hour, data->trd_hour, sizeof(data->trd_hour));	// 시간대,HHMM-HHMM
...
int ParseI0016_FBC1(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_2* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FBC1(원/달러 전일 기일물별 체결현황1)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,*기일물구분 참조
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
...
int ParseI0016_FBC2(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_3* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FBC2(원/달러 전일 기일물별 체결현황2)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,*기일물구분 참조
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
...
int ParseI0016_FBD1(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_4* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FBD1(환율고시 재정환율 현황)
	out->informed_date = Str2Int(data->informed_date, sizeof(data->informed_date));	// 고시일,YYYYMMDD
	memcpy(out->currency_name, data->currency_name, sizeof(data->currency_name));	// 통화명,주)통화명 참조
	out->cross_rate = Str2Dbl(data->cross_rate, sizeof(data->cross_rate), 5);	// CROSS RATE,9999.99999
	out->fx_rate = Str2Dbl(data->fx_rate, sizeof(data->fx_rate), 2);	// 환율,9999.99
...
int ParseI0016_FBE1(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_5* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FBE1(SWAP 전일자 기일물별 체결현황)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,주)기일물구분 참조
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
...
int ParseI0016_FBF1(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_6* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FBF1(원/달러 선도환율 : 통화선도환율방식)
	out->std_date = Str2Int(data->std_date, sizeof(data->std_date));	// 기준일,YYYYMMDD
	memcpy(out->fixed_date_clsf_0, data->fixed_date_clsf_0, sizeof(data->fixed_date_clsf_0));	// 기일물구분,주)선도환율기일물구분 참조
	out->rate_0 = Str2Dbl(data->rate_0, sizeof(data->rate_0), 2);	// RATE,9999.99
	memcpy(out->fixed_date_clsf_1, data->fixed_date_clsf_1, sizeof(data->fixed_date_clsf_1));	// 기일물구분,주)선도환율기일물구분 참조
	out->rate_1 = Str2Dbl(data->rate_1, sizeof(data->rate_1), 2);	// RATE,9999.99
	memcpy(out->fixed_date_clsf_2, data->fixed_date_clsf_2, sizeof(data->fixed_date_clsf_2));	// 기일물구분,주)선도환율기일물구분 참조
	out->rate_2 = Str2Dbl(data->rate_2, sizeof(data->rate_2), 2);	// RATE,9999.99
	memcpy(out->fixed_date_clsf_3, data->fixed_date_clsf_3, sizeof(data->fixed_date_clsf_3));	// 기일물구분,주)선도환율기일물구분 참조
...
int ParseI0016_FBF2(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_7* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FBF2(원/달러 선도 환율 : 스왑포인트 방식))
	out->std_date = Str2Int(data->std_date, sizeof(data->std_date));	// 기준일,YYYYMMDD
	memcpy(out->fixed_date_clsf_0, data->fixed_date_clsf_0, sizeof(data->fixed_date_clsf_0));	// 기일물구분,주)선도환율기일물구분 참조
	out->rate_0 = Str2Dbl(data->rate_0, sizeof(data->rate_0), 2);	// RATE,9999.99
	memcpy(out->fixed_date_clsf_1, data->fixed_date_clsf_1, sizeof(data->fixed_date_clsf_1));	// 기일물구분,주)선도환율기일물구분 참조
...
int ParseI0016_FBG1(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_8* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FBG1(월평균 기준환율)
	memcpy(out->informed_date, data->informed_date, sizeof(data->informed_date));	// 고시일,YYYYMMDD
	out->std_year_month = Str2Dbl(data->std_year_month, sizeof(data->std_year_month), 2);	// 기준년월,9999.99
	out->avg_fx_rate = Str2Dbl(data->avg_fx_rate, sizeof(data->avg_fx_rate), 2);	// 평균기준환율,9999.99
...
int ParseI0016_CBA1(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_9* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,CBA1(외화 전일 거래요약1)
	memcpy(out->trd_date, data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->currency_name, data->currency_name, sizeof(data->currency_name));	// 통화명,주)통화명 참조
...
int ParseI0016_CBA2(const double& timestamp, const char* in_buff, DF_USD_BATCH_DATA_10* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,CBA2(외화 전일 거래요약2)
	memcpy(out->trd_date, data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->currency_name, data->currency_name, sizeof(data->currency_name));	// 통화명,주)통화명 참조
...
int ParseI1016_FOC1(const double& timestamp, const char* in_buff, DF_USD_REAL_TIME_DATA_1* out) {
	...
	out->timestamp = timestamp;
	memcpy(out->data_clsf, data->data_clsf, sizeof(data->data_clsf));	// DATA구분,I1
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FOC1(원/달러 시간대별 거래현황)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->trd_hour, data->trd_hour, sizeof(data->trd_hour));	// 시간대,HHMM-HHMM
...
int ParseI1016_FOD1(const double& timestamp, const char* in_buff, DF_USD_REAL_TIME_DATA_2* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FOD1(원/달러 기일물별 접수현황)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,주)기일물구분참조
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
...
int ParseI1016_FOE1(const double& timestamp, const char* in_buff, DF_USD_REAL_TIME_DATA_3* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FOE1(원/달러 기일물별 체결현황) (접수->체결)변경
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,주)기일물구분참조
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
...
int ParseI1016_HOE1(const double& timestamp, const char* in_buff, DF_CNH_REAL_TIME_DATA* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,HOE1(원/위안 기일물별 체결현황)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,주)기일물구분참조
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
	out->low_rate = Str2Dbl(data->low_rate, sizeof(data->low_rate), 1);	// LOW RATE,9999.9
...
int ParseI1016_FOF1(const double& timestamp, const char* in_buff, DF_USD_REAL_TIME_DATA_4* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FOF1(SWAP 기일물별 접수현황)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,주)기일물구분참조
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
...
int ParseI1016_FOG1(const double& timestamp, const char* in_buff, DF_USD_REAL_TIME_DATA_5* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FOG1(SWAP 기일물별 체결현황)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,주)기일물구분참조
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
	out->low_rate = Str2Int(data->low_rate, sizeof(data->low_rate));	// LOW RATE,±9999
...
int ParseI1016_UOD1(const double& timestamp, const char* in_buff, DF_USD_REAL_TIME_DATA_8* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,UOD1(원달러 SPOT 대고객 환율호가)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,주)기일물 구분
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
	memcpy(out->bank_name, data->bank_name, sizeof(data->bank_name));	// 은행명,영문은행명
	out->bid_best = Str2Dbl(data->bid_best, sizeof(data->bid_best), 1);	// BID BEST,9999.9
	out->offer_best = Str2Dbl(data->offer_best, sizeof(data->offer_best), 1);	// OFFER BEST,9999.9
	out->low_rate = Str2Dbl(data->low_rate, sizeof(data->low_rate), 1);	// LOW RATE,9999.9
	out->high_rate = Str2Dbl(data->high_rate, sizeof(data->high_rate), 1);	// HIGH RATE,9999.9
...
int ParseI1016_COA1(const double& timestamp, const char* in_buff, DF_USD_REAL_TIME_DATA_7* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,COA1(외화 기일물별 접수현황)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->currency_name, data->currency_name, sizeof(data->currency_name));	// 통화명,주)통화명 참조
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물 구분,주)기일물 구분
...
int ParseI1016_FOQ1(const double& timestamp, const char* in_buff, DF_USD_REAL_TIME_DATA_6* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,FOQ1(현물환 시간대별 BID-OFFER 호가 현황)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->td_hour, data->td_hour, sizeof(data->td_hour));	// 시간대,HHMM
	out->spread_bid_prc = Str2Dbl(data->spread_bid_prc, sizeof(data->spread_bid_prc), 1);	// BID 호가(SP),9999.9
	out->spread_ask_prc = Str2Dbl(data->spread_ask_prc, sizeof(data->spread_ask_prc), 1);	// OFFER 호가(SP),9999.9
...
int ParseI2016(const double& timestamp, const char* in_buff, DF_COM_FUT_POLLING_DATA* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->send_time, data->send_time, sizeof(data->send_time));	// 전송시간,HHSS
...
int ParseI1016_UOE1(const double& timestamp, const char* in_buff, DF_USD_REAL_TIME_DATA_9* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	memcpy(out->job_clsf_code, data->job_clsf_code, sizeof(data->job_clsf_code));	// 업무구분코드,UOE1(원달러 SPOT 30분단위 행간 환율체결가)
	out->trd_date = Str2Int(data->trd_date, sizeof(data->trd_date));	// 거래일,YYYYMMDD
	memcpy(out->fixed_date_clsf, data->fixed_date_clsf, sizeof(data->fixed_date_clsf));	// 기일물구분,SP
	out->maturity_date = Str2Int(data->maturity_date, sizeof(data->maturity_date));	// 만기일(결제일),YYYYMMDD
	out->exe_t = Str2Int(data->exe_t, sizeof(data->exe_t));	// 체결시간,HHMM(0930, 1000,1030,1100,1130,1200,1230,\n1300,1330,1400, 1430, 1500)
	out->exe_prc = Str2Dbl(data->exe_prc, sizeof(data->exe_prc), 1);	// 체결가,9999.9
...
int H8016_1000(const double& timestamp, const char* in_buff, DF_LEAN_HOG_REP_PRC_INDEX_DATA* out) {
	...
	out->timestamp = timestamp;
	memcpy(out->data_clsf, data->data_clsf, sizeof(data->data_clsf));	// DATA구분,H8
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	out->data_type = data->data_type[0];	//데이터 타입,D:돈육 대표가격 지수
	memcpy(out->inform_date_yest, data->inform_date_yest, sizeof(data->inform_date_yest));	// 공시일자-전일
	memcpy(out->inform_date_close_yest, data->inform_date_close_yest, sizeof(data->inform_date_close_yest));	// 공시일자-직전일
	out->accum_mrk_nbr = Str2Int(data->accum_mrk_nbr, sizeof(data->accum_mrk_nbr));	// 집계도매시장수,개수
	out->accum_prd_nbr = Str2Int(data->accum_prd_nbr, sizeof(data->accum_prd_nbr));	// 집계두수,두
	out->trd_weight_total = Str2Dbl(data->trd_weight_total, sizeof(data->trd_weight_total), 0);	// 거래 도체중 합계,kg
	out->trd_prc_total = Str2Dbl(data->trd_prc_total, sizeof(data->trd_prc_total), 0);	// 거래 가격 합계,원
	out->rep_prc = Str2Int(data->rep_prc, sizeof(data->rep_prc));	// 대표 가격,원/kg
...
int H8016_REAL(const double& timestamp, const char* in_buff, DF_LEAN_HOG_REAL_TIME_PRC_INDEX_DATA* out) {
	...
	out->timestamp = timestamp;
	memcpy(out->data_cslf, data->data_cslf, sizeof(data->data_cslf));	// DATA구분,H8
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	out->data_type = data->data_type[0];	//데이터 타입,L:실시간 현물가격 지수
	memcpy(out->inform_time, data->inform_time, sizeof(data->inform_time));	// 공시시간
	memcpy(out->inform_date_yest, data->inform_date_yest, sizeof(data->inform_date_yest));	// 공시일자-전일
	memcpy(out->inform_date_today, data->inform_date_today, sizeof(data->inform_date_today));	// 공시일자-당일
	out->accum_mrk_nbr = Str2Int(data->accum_mrk_nbr, sizeof(data->accum_mrk_nbr));	// 집계 도매시장 수,개수
	out->accum_prd_nbr = Str2Int(data->accum_prd_nbr, sizeof(data->accum_prd_nbr));	// 경락두수,두
	out->trd_weight_total = Str2Dbl(data->trd_weight_total, sizeof(data->trd_weight_total), 0);	// 거래 도체중 합계,kg
	out->trd_prc_total = Str2Dbl(data->trd_prc_total, sizeof(data->trd_prc_total), 0);	// 거래 가격 합계,원
	out->under_asset_prc = Str2Int(data->under_asset_prc, sizeof(data->under_asset_prc));	// 현물 가격,원/kg
...
int H8016_1800(const double& timestamp, const char* in_buff, DF_LEAN_HOG_CLOSE_PRICE_INDEX_DATA* out) {
	...
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:상품선물,\n최종 record '99' set
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,6
	out->data_type = data->data_type[0];	//데이터 타입,E:실시간 현물가격 마감지수
	memcpy(out->inform_date_today, data->inform_date_today, sizeof(data->inform_date_today));	// 공시일자 당일
	memcpy(out->inform_date_yest, data->inform_date_yest, sizeof(data->inform_date_yest));	// 공시일자 전일
	out->accum_mrk_nbr = Str2Int(data->accum_mrk_nbr, sizeof(data->accum_mrk_nbr));	// 집계 도매시장 수
	out->accum_prd_nbr = Str2Int(data->accum_prd_nbr, sizeof(data->accum_prd_nbr));	// 집계 두수,개수
	out->trd_weight_total = Str2Dbl(data->trd_weight_total, sizeof(data->trd_weight_total), 0);	// 거래 도체중 합계,두
	out->trd_prc_total = Str2Dbl(data->trd_prc_total, sizeof(data->trd_prc_total), 0);	// 거래 가격 합계,kg
	out->init_prc = Str2Int(data->init_prc, sizeof(data->init_prc));	// 시작 가격,원
	out->high_prc = Str2Int(data->high_prc, sizeof(data->high_prc));	// 최고 가격,원/kg
	out->low_prc = Str2Int(data->low_prc, sizeof(data->low_prc));	// 최저 가격
	out->close_prc = Str2Int(data->close_prc, sizeof(data->close_prc));	// 최종 가격
