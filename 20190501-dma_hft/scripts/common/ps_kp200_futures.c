...
// K200선물_경쟁+협의합산
int ParseC4014(const double& timestamp, const char* in_buff, DF_K200_FUT_COMP_BLOCK* out) {
	K200_FUT_COMP_BLOCK* data = (K200_FUT_COMP_BLOCK*)in_buff;

	out->timestamp = timestamp;
	memcpy(out->data_clsf, data->data_clsf, sizeof(data->data_clsf));	// DATA구분,C4
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:KOSPI200지수선물
...
// K200선물_체결
int ParseA3014(const double& timestamp, const char* in_buff, DF_K200_FUT_EXEC* out) {
	K200_FUT_EXEC* data = (K200_FUT_EXEC*)in_buff;
	...
	out->cur_prc = Str2SignDouble(data->cur_prc_sign, sizeof(data->cur_prc_sign) + sizeof(data->cur_prc), 2);	// 현재가격,예) 999.99
	...
	out->rct_prd_prc = Str2Dbl(data->rct_prd_prc, sizeof(data->rct_prd_prc), 2);	// 최근월물의제약정가격,예) 999.99
	out->nxt_prd_prc = Str2Dbl(data->nxt_prd_prc, sizeof(data->nxt_prd_prc), 2);	// 원월물의제약정가격,예) 999.99
	out->init_prc = Str2SignDouble(data->init_prc_sign, sizeof(data->init_prc_sign) + sizeof(data->init_prc), 2);	// 시가,예) 999.99
	out->high_prc = Str2SignDouble(data->high_prc_sign, sizeof(data->high_prc_sign) + sizeof(data->high_prc), 2);	// 고가,예) 999.99
	out->low_prc = Str2SignDouble(data->low_prc_sign, sizeof(data->low_prc_sign) + sizeof(data->low_prc), 2);	// 저가,예) 999.99
	out->pri_prc = Str2SignDouble(data->pri_prc_sign, sizeof(data->pri_prc_sign) + sizeof(data->pri_prc), 2);	// 직전가격,예) 999.99
	...
	out->real_t_high_prc = Str2SignDouble(data->real_t_high_prc_sign, sizeof(data->real_t_high_prc_sign) + sizeof(data->real_t_high_prc), 2);	// 실시간상한가,예) 999.99 개발가이드 참조
	out->real_t_low_prc = Str2SignDouble(data->real_t_low_prc_sign, sizeof(data->real_t_low_prc_sign) + sizeof(data->real_t_low_prc), 2);	// 실시간하한가,예) 999.99 개발가이드 참조
...
// K200선물_우선호가
int ParseB6014(const double& timestamp, const char* in_buff, DF_K200_FUT_LIMIT_ORD* out) {
	K200_FUT_LIMIT_ORD* data = (K200_FUT_LIMIT_ORD*)in_buff;
	...
	out->l_lvl_1_prc = Str2SignDouble(data->l_lvl_1_sign, sizeof(data->l_lvl_1_sign) + sizeof(data->l_lvl_1_prc), 2);	// 매수1단계우선호가가격,예) 999.99
	out->l_lvl_1_q = Str2Int(data->l_lvl_1_q, sizeof(data->l_lvl_1_q));	// 매수1단계우선호가잔량
	out->l_lvl_2_prc = Str2SignDouble(data->l_lvl_2_sign, sizeof(data->l_lvl_2_sign) + sizeof(data->l_lvl_2_prc), 2);	// 매수2단계우선호가가격,예) 999.99
	out->l_lvl_2_q = Str2Int(data->l_lvl_2_q, sizeof(data->l_lvl_2_q));	// 매수2단계우선호가잔량
	out->l_lvl_3_prc = Str2SignDouble(data->l_lvl_3_sign, sizeof(data->l_lvl_3_sign) + sizeof(data->l_lvl_3_prc), 2);	// 매수3단계우선호가가격,예) 999.99
	out->l_lvl_3_q = Str2Int(data->l_lvl_3_q, sizeof(data->l_lvl_3_q));	// 매수3단계우선호가잔량
	out->l_lvl_4_prc = Str2SignDouble(data->l_lvl_4_sign, sizeof(data->l_lvl_4_sign) + sizeof(data->l_lvl_4_prc), 2);	// 매수4단계우선호가가격,예) 999.99
	out->l_lvl_4_q = Str2Int(data->l_lvl_4_q, sizeof(data->l_lvl_4_q));	// 매수4단계우선호가잔량
	out->l_lvl_5_prc = Str2SignDouble(data->l_lvl_5_sign, sizeof(data->l_lvl_5_sign) + sizeof(data->l_lvl_5_prc), 2);	// 매수5단계우선호가가격,예) 999.99
	...
	out->s_lvl_1_prc = Str2SignDouble(data->s_lvl_1_sign, sizeof(data->s_lvl_1_sign) + sizeof(data->s_lvl_1_prc), 2);	// 매도1단계우선호가가격,예) 999.99
	out->s_lvl_1_q = Str2Int(data->s_lvl_1_q, sizeof(data->s_lvl_1_q));	// 매도1단계우선호가잔량
	out->s_lvl_2_prc = Str2SignDouble(data->s_lvl_2_sign, sizeof(data->s_lvl_2_sign) + sizeof(data->s_lvl_2_prc), 2);	// 매도2단계우선호가가격,예) 999.99
	out->s_lvl_2_q = Str2Int(data->s_lvl_2_q, sizeof(data->s_lvl_2_q));	// 매도2단계우선호가잔량
	out->s_lvl_3_prc = Str2SignDouble(data->s_lvl_3_sign, sizeof(data->s_lvl_3_sign) + sizeof(data->s_lvl_3_prc), 2);	// 매도3단계우선호가가격,예) 999.99
	out->s_lvl_3_q = Str2Int(data->s_lvl_3_q, sizeof(data->s_lvl_3_q));	// 매도3단계우선호가잔량
	out->s_lvl_4_prc = Str2SignDouble(data->s_lvl_4_sign, sizeof(data->s_lvl_4_sign) + sizeof(data->s_lvl_4_prc), 2);	// 매도4단계우선호가가격,예) 999.99
	out->s_lvl_4_q = Str2Int(data->s_lvl_4_q, sizeof(data->s_lvl_4_q));	// 매도4단계우선호가잔량
	out->s_lvl_5_prc = Str2SignDouble(data->s_lvl_5_sign, sizeof(data->s_lvl_5_sign) + sizeof(data->s_lvl_5_prc), 2);	// 매도5단계우선호가가격,예) 999.99
	...
	out->exp_prc = Str2SignDouble(data->exp_prc_sign, sizeof(data->exp_prc_sign) + sizeof(data->exp_prc), 2);	// 예상체결가격,예) 999.99\n 단일가매매시만 제공\n (장상태구분코드:10,20,30,80)
...
// K200선물_체결_우선호가
int ParseG7014(const double& timestamp, const char* in_buff, DF_K200_FUT_EXEC_LIMIT_ORD* out) {
	K200_FUT_EXEC_LIMIT_ORD* data = (K200_FUT_EXEC_LIMIT_ORD*)in_buff;
	...
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목일련번호
	memcpy(out->brd_id, data->brd_id, sizeof(data->brd_id));	// 보드ID,코드값모음참조
	out->cur_price = Str2SignDouble(data->cur_prc_sign, sizeof(data->cur_prc_sign) + sizeof(data->cur_price), 2);	// 현재가격,예) 999.99
	out->exe_q = Str2Int(data->exe_q, sizeof(data->exe_q));	// 체결수량
	memcpy(out->ss_id, data->ss_id, sizeof(data->ss_id));	// 세션ID,코드값모음참조
	memcpy(out->exe_t, data->exe_t, sizeof(data->exe_t));	// 체결시각
	out->rct_prd_prc = Str2Dbl(data->rct_prd_prc, sizeof(data->rct_prd_prc), 2);	// 최근월물의제약정가격,예) 999.99
...
// K200선물_종목마감
int ParseA6014(const double& timestamp, const char* in_buff, DF_K200_FUT_CLOSE_PRODUCT* out) {
	K200_FUT_CLOSE_PRODUCT* data = (K200_FUT_CLOSE_PRODUCT*)in_buff;
	...
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목일련번호
	memcpy(out->brd_id, data->brd_id, sizeof(data->brd_id));	// 보드ID,코드값모음참조
	out->close_prc = Str2SignDouble(data->close_prc_sign, sizeof(data->close_prc_sign) + sizeof(data->close_prc), 2);	// 종가,* 거래무 종목의 경우 기준가 set, 예) 999.99
	...
	out->s_lvl_5_prc = Str2SignDouble(data->s_lvl_5_sign, sizeof(data->s_lvl_5_sign) + sizeof(data->s_lvl_5_prc), 2);	// 매도5단계우선호가가격,예) 999.99
	out->s_lvl_5_q = Str2Int(data->s_lvl_5_q, sizeof(data->s_lvl_5_q));	// 매도5단계우선호가잔량
	out->total_l_trd_cnt = Str2Dbl(data->total_l_trd_cnt, sizeof(data->total_l_trd_cnt), 2);	// 매수유효호가건수,예) 999.99
	out->l_lvl_1_trd_cnt = Str2Int(data->l_lvl_1_trd_cnt, sizeof(data->l_lvl_1_trd_cnt));	// 매수1단계우선호가건수
	out->l_lvl_2_trd_cnt = Str2Int(data->l_lvl_2_trd_cnt, sizeof(data->l_lvl_2_trd_cnt));	// 매수2단계우선호가건수
	out->l_lvl_3_trd_cnt = Str2Int(data->l_lvl_3_trd_cnt, sizeof(data->l_lvl_3_trd_cnt));	// 매수3단계우선호가건수
	out->l_lvl_4_trd_cnt = Str2Int(data->l_lvl_4_trd_cnt, sizeof(data->l_lvl_4_trd_cnt));	// 매수4단계우선호가건수
...
// K200선물_장운영TS
int ParseA7014(const double& timestamp, const char* in_buff, DF_K200_FUT_MRK_OP_TS* out) {
	K200_FUT_MRK_OP_TS* data = (K200_FUT_MRK_OP_TS*)in_buff;
	...
	out->s_lvl_4_trd_cnt = Str2Int(data->s_lvl_4_trd_cnt, sizeof(data->s_lvl_4_trd_cnt));	// 매도4단계우선호가건수
	out->s_lvl_5_trd_cnt = Str2Int(data->s_lvl_5_trd_cnt, sizeof(data->s_lvl_5_trd_cnt));	// 매도5단계우선호가건수
	out->real_t_high_prc = Str2SignDouble(data->real_t_high_prc_sign, sizeof(data->real_t_high_prc_sign) + sizeof(data->real_t_high_prc), 2);	// 실시간상한가,예) 999.99 개발가이드참조
	out->real_t_low_prc = Str2SignDouble(data->real_t_low_prc_sign, sizeof(data->real_t_low_prc_sign) + sizeof(data->real_t_low_prc), 2);	// 실시간하한가,예) 999.99 개발가이드참조
...
// K200선물_마감
int ParseH0014(const double& timestamp, const char* in_buff, DF_K200_FUT_CLOSE* out) {
	K200_FUT_CLOSE* data = (K200_FUT_CLOSE*)in_buff;
...
// K200선물_장운영스케줄공개
int ParseM4014(const double& timestamp, const char* in_buff, DF_K200_FUT_MRK_OP_SCHD* out) {
	K200_FUT_MRK_OP_SCHD* data = (K200_FUT_MRK_OP_SCHD*)in_buff;
	...
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목일련번호,개별종목 장운영조치시 등록
	memcpy(out->prd_id, data->prd_id, sizeof(data->prd_id));	// 상품ID,코드값모음참조 : space처리
	memcpy(out->mrk_prd_id, data->mrk_prd_id, sizeof(data->mrk_prd_id));	// 장운영상품그룹ID,FKI : KRX 파생 선물 코스피200
	memcpy(out->brd_id, data->brd_id, sizeof(data->brd_id));	// 보드ID,코드값모음참조
	memcpy(out->brd_event_id, data->brd_event_id, sizeof(data->brd_event_id));	// 보드이벤트ID,코드값모음참조
...
// K200선물_배분정보
int ParseO6014(const double& timestamp, const char* in_buff, DF_K200_FUT_DIST_INFO* out) {
	K200_FUT_DIST_INFO* data = (K200_FUT_DIST_INFO*)in_buff;
...
int ParseQ2014(const double& timestamp, const char* in_buff, DF_K200_FUT_REAL_HIGH_LOW_LIMIT_ON_OFF* out) {
	K200_FUT_REAL_HIGH_LOW_LIMIT_ON_OFF* data = (K200_FUT_REAL_HIGH_LOW_LIMIT_ON_OFF*)in_buff;
...
// K200선물_가격제한폭확대발동
int ParseV1014(const double& timestamp, const char* in_buff, DF_K200_FUT_PRC_LIMIT_ON* out) {
	K200_FUT_PRC_LIMIT_ON* data = (K200_FUT_PRC_LIMIT_ON*)in_buff;
	...
	out->high_prc = Str2SignDouble(data->high_prc_sign, sizeof(data->high_prc_sign) + sizeof(data->high_prc), 2);	// 상한가,적용된 상한가\n예)999.99
	out->low_prc = Str2SignDouble(data->low_prc_sign, sizeof(data->low_prc_sign) + sizeof(data->low_prc), 2);	// 하한가,적용된 하한가\n예)999.99
...
// K200선물_미결제약정수량 데이터
int ParseH2014(const double& timestamp, const char* in_buff, DF_K200_FUT_OPEN_INTEREST_DATA* out) {
	K200_FUT_OPEN_INTEREST_DATA* data = (K200_FUT_OPEN_INTEREST_DATA*)in_buff;
	...
	out->open_interest_clsf = data->open_interest_clsf[0];	//미결제약정구분,전일 확정 : "M0" (07:40분경 제공)\n당일 잠정 : "M1" (장중 특정간격으로 변동종목\n		 송신, 10초)\n당일 확정 : "M2" (당일 전종목 송신,\n		 장종료 1시간후 제공)
...
// K200선물_정산가격 데이터
int ParseH3014(const double& timestamp, const char* in_buff, DF_K200_FUT_SETT_PRC_DATA* out) {
	K200_FUT_SETT_PRC_DATA* data = (K200_FUT_SETT_PRC_DATA*)in_buff;

	out->timestamp = timestamp;
	memcpy(out->data_clsf, data->data_clsf, sizeof(data->data_clsf));	// DATA구분,H3
	memcpy(out->info_clsf, data->info_clsf, sizeof(data->info_clsf));	// 정보구분,01:KOSPI200지수선물
	out->mrk_clsf = data->mrk_clsf[0];	//시장구분,4
	memcpy(out->isin_clsf, data->isin_clsf, sizeof(data->isin_clsf));	// 종목 코드,최종 record '999999999999' set
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목SEQ
...
// K200선물_투자자 데이터
int ParseH1014(const double& timestamp, const char* in_buff, DF_K200_FUT_INVESTOR_DATA* out) {
	K200_FUT_INVESTOR_DATA* data = (K200_FUT_INVESTOR_DATA*)in_buff;
	...
	memcpy(out->prd_id, data->prd_id, sizeof(data->prd_id));	// 상품ID,코드값모음참조 - KRDRVFUK2I
...
int ParseB2014(const double& timestamp, const char* in_buff, DF_K200_FUT_RECOVERY_DATA* out) {
	K200_FUT_RECOVERY_DATA* data = (K200_FUT_RECOVERY_DATA*)in_buff;
	...
	memcpy(out->isin_clsf, data->isin_clsf, sizeof(data->isin_clsf));	// 종목코드
	out->stk_snr = Str2Int(data->stk_snr, sizeof(data->stk_snr));	// 종목일련번호
	out->cur_prc = Str2SignDouble(data->cur_prc_sign, sizeof(data->cur_prc_sign) + sizeof(data->cur_prc), 2);	// 현재가,예) 999.99
	out->rct_prd_prc = Str2Dbl(data->rct_prd_prc, sizeof(data->rct_prd_prc), 2);	// 최근월물 의제약정가격,예) 999.99
	out->nxt_prd_prc = Str2Dbl(data->nxt_prd_prc, sizeof(data->nxt_prd_prc), 2);	// 원월물 의제약정가격,예) 999.99
	out->init_prc = Str2SignDouble(data->init_prc_sign, sizeof(data->init_prc_sign) + sizeof(data->init_prc), 2);	// 시가,예) 999.99
	out->high_prc = Str2SignDouble(data->high_prc_sign, sizeof(data->high_prc_sign) + sizeof(data->high_prc), 2);	// 고가,예) 999.99
	out->low_prc = Str2SignDouble(data->low_prc_sign, sizeof(data->low_prc_sign) + sizeof(data->low_prc), 2);	// 저가,예) 999.99
	...
	out->accm_trd_amt = Str2Dbl(data->accm_trd_amt, sizeof(data->accm_trd_amt), 0);	// 누적거래대금,단위:천원
	out->l_or_s = data->l_or_s[0];	//최종매도매수구분코드,1:매도,2:매수(최종으로 들어온 호가의 매도매수구분값)
	out->s_lvl_1_prc = Str2SignDouble(data->s_lvl_1_sign, sizeof(data->s_lvl_1_sign) + sizeof(data->s_lvl_1_prc), 2);	// 매도1호가,예) 999.99
	...
	out->l_lvl_1_prc = Str2SignDouble(data->l_lvl_1_sign, sizeof(data->l_lvl_1_sign) + sizeof(data->l_lvl_1_prc), 2);	// 매수1호가,예) 999.99
	...
	out->s_lvl_2_prc = Str2SignDouble(data->s_lvl_2_sign, sizeof(data->s_lvl_2_sign) + sizeof(data->s_lvl_2_prc), 2);	// 매도2호가,예) 999.99
	...
	out->l_lvl_2_prc = Str2SignDouble(data->l_lvl_2_sign, sizeof(data->l_lvl_2_sign) + sizeof(data->l_lvl_2_prc), 2);	// 매수2호가,예) 999.99
	...
	out->s_lvl_3_prc = Str2SignDouble(data->s_lvl_3_sign, sizeof(data->s_lvl_3_sign) + sizeof(data->s_lvl_3_prc), 2);	// 매도3호가,예) 999.99
	...
	out->l_lvl_3_prc = Str2SignDouble(data->l_lvl_3_sign, sizeof(data->l_lvl_3_sign) + sizeof(data->l_lvl_3_prc), 2);	// 매수3호가,예) 999.99
	...
	out->s_lvl_4_prc = Str2SignDouble(data->s_lvl_4_sign, sizeof(data->s_lvl_4_sign) + sizeof(data->s_lvl_4_prc), 2);	// 매도4호가,예) 999.99
	...
	out->l_lvl_4_prc = Str2SignDouble(data->l_lvl_4_sign, sizeof(data->l_lvl_4_sign) + sizeof(data->l_lvl_4_prc), 2);	// 매수4호가,예) 999.99
	...
	out->s_lvl_5_prc = Str2SignDouble(data->s_lvl_5_sign, sizeof(data->s_lvl_5_sign) + sizeof(data->s_lvl_5_prc), 2);	// 매도5호가,예) 999.99
	...
	out->l_lvl_5_prc = Str2SignDouble(data->l_lvl_5_sign, sizeof(data->l_lvl_5_sign) + sizeof(data->l_lvl_5_prc), 2);	// 매수5호가,예) 999.99
...
// K200선물_종목정보 데이터
int ParseA0014(const double& timestamp, const char* in_buff, DF_K200_FUT_PRD_INFO_DATA* out) {
	K200_FUT_PRD_INFO_DATA* data = (K200_FUT_PRD_INFO_DATA*)in_buff;
	...
	memcpy(out->under_asset_id, data->under_asset_id, sizeof(data->under_asset_id));	// 기초자산ID,예) K2I:KOSPI200지수, STI:스타지수,\n   Sxx:개별주식, BM3:신3년국채,\n   BM5:신5년국채, BMA:신10년국채, \n   CDR:CD, USD:미국달러, JPY:일본엔,\n   EUR:유럽유로,\n   GLD:금, LHG:돈육\n20140303
...
// K200선물_Polling 데이터
int ParseI2014(const double& timestamp, const char* in_buff, DF_K200_FUT_POLLING_DATA* out) {
	K200_FUT_POLLING_DATA* data = (K200_FUT_POLLING_DATA*)in_buff;
...
// K200선물_SPACE
int ParseG9014(const double& timestamp, const char* in_buff, DF_K200_FUT_SPACE* out) {
	K200_FUT_SPACE* data = (K200_FUT_SPACE*)in_buff;
