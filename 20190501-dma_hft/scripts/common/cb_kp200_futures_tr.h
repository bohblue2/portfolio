...
// K200선물_경쟁+협의합산
typedef struct _K200_FUT_COMP_BLOCK {
...
// K200선물_종목마감
typedef struct _K200_FUT_CLOSE_PRODUCT {
	...
	char s_lvl_5_prc[5];	// 매도5단계우선호가가격,예) 999.99
	char s_lvl_5_q[6];	// 매도5단계우선호가잔량
	char total_l_trd_cnt[5];	// 매수유효호가건수,예) 999.99
	char l_lvl_1_trd_cnt[4];	// 매수1단계우선호가건수
	char l_lvl_2_trd_cnt[4];	// 매수2단계우선호가건수
	char l_lvl_3_trd_cnt[4];	// 매수3단계우선호가건수
	char l_lvl_4_trd_cnt[4];	// 매수4단계우선호가건수
	...
	char s_lvl_5_trd_cnt[4];	// 매도5단계우선호가건수
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_CLOSE_PRODUCT;


// K200선물_장운영TS
typedef struct _K200_FUT_MRK_OP_TS {
...
// K200선물_마감
typedef struct _K200_FUT_CLOSE {
...
// K200선물_장운영스케줄공개
typedef struct _K200_FUT_MRK_OP_SCHD {
	char data_clsf[2];	// DATA구분,M4
	char info_clsf[2];	// 정보구분,01:KOSPI200지수선물
	char mrk_clsf[1];	// 시장구분,4
	char isin_clsf[12];	// 종목코드,개별종목 장운영조치시 등록
	char stk_snr[3];	// 종목일련번호,개별종목 장운영조치시 등록
	char prd_id[11];	// 상품ID,코드값모음참조 : space처리
	char mrk_prd_id[3];	// 장운영상품그룹ID,FKI : KRX 파생 선물 코스피200
	char brd_id[2];	// 보드ID,코드값모음참조
	char brd_event_id[3];	// 보드이벤트ID,코드값모음참조
	...
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_MRK_OP_SCHD;


// K200선물_배분정보
typedef struct _K200_FUT_DIST_INFO {
...
typedef struct _K200_FUT_REAL_HIGH_LOW_LIMIT_ON_OFF {
	...
	char real_t_low_prc[5];	// 실시간하한가,예)999.99 개발가이드참조
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_REAL_HIGH_LOW_LIMIT_ON_OFF;


// K200선물_가격제한폭확대발동
typedef struct _K200_FUT_PRC_LIMIT_ON {
	...
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_PRC_LIMIT_ON;


// K200선물_미결제약정수량 데이터
typedef struct _K200_FUT_OPEN_INTEREST_DATA {
	char data_clsf[2];	// DATA구분,H2
	char info_clsf[2];	// 정보구분,01:KOSPI200지수선물
	char mrk_clsf[1];	// 시장구분,4
	char isin_clsf[12];	// 종목코드,최종 record '999999999999' set
	char stk_snr[2];	// 종목SEQ
	...
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_OPEN_INTEREST_DATA;


// K200선물_정산가격 데이터
typedef struct _K200_FUT_SETT_PRC_DATA {
	char data_clsf[2];	// DATA구분,H3
	char info_clsf[2];	// 정보구분,01:KOSPI200지수선물
	char mrk_clsf[1];	// 시장구분,4
	char isin_clsf[12];	// 종목 코드,최종 record '999999999999' set
	char stk_snr[2];	// 종목SEQ
	...
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_SETT_PRC_DATA;


// K200선물_투자자 데이터
typedef struct _K200_FUT_INVESTOR_DATA {
	...
	char prd_id[11];	// 상품ID,코드값모음참조 - KRDRVFUK2I
	...
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_INVESTOR_DATA;


// K200선물_시세Recovery 데이터
typedef struct _K200_FUT_RECOVERY_DATA {
	...
	char rct_prd_prc[5];	// 최근월물 의제약정가격,예) 999.99
	char nxt_prd_prc[5];	// 원월물 의제약정가격,예) 999.99
	char init_prc_sign[1];	// 시가부호
	char init_prc[5];	// 시가,예) 999.99
	char high_prc_sign[1];	// 고가부호
	char high_prc[5];	// 고가,예) 999.99
	...
	char total_s_limit_ord_q[6];	// 매도 총호가수량
	char total_s_trd_cnt[5];	// 매도유효호가건수
	char total_l_limit_ord_q[6];	// 매수 총호가수량
	char total_l_trd_cnt[5];	// 매수유효호가건수
	char ss_id[2];	// 세션ID,코드값모음참조
	char brd_id[2];	// 보드ID,코드값모음참조
	...
	char real_t_low_prc[5];	// 실시간하한가,예) 999.99 개발가이드 참조
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_RECOVERY_DATA;


// K200선물_종목정보 데이터
typedef struct _K200_FUT_PRD_INFO_DATA {
	...
	char under_asset_id[3];	// 기초자산ID,예) K2I:KOSPI200지수, STI:스타지수,\n   Sxx:개별주식, BM3:신3년국채,\n   BM5:신5년국채, BMA:신10년국채, \n   CDR:CD, USD:미국달러, JPY:일본엔,\n   EUR:유럽유로,\n   GLD:금, LHG:돈육\n20140303
	...
	char filler[24];	// Filler,170 => 134, 20090831 \n134 => 133, 20121217 \n133 => 110, 20140303 \n110 => 83, 20140901 \n83 => 27, 20150615 \n27 => 24, 20150720
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_PRD_INFO_DATA;


// K200선물_Polling 데이터
typedef struct _K200_FUT_POLLING_DATA {
	...
	char ff[1];	// End of Text, (0xFF)
} K200_FUT_POLLING_DATA;


// K200선물_SPACE
typedef struct _K200_FUT_SPACE {
